...
class RetryManager:
            ...
            for i, msg in enumerate(list(messages)):
                try:
                    ...
                    if time_since_last_retry < wait_seconds:
                        # 还没到重试时间，跳过
                        logger.debug(
                            f"消息重试冷却中，还需等待 {(wait_seconds - time_since_last_retry) / 3600:.1f} 小时: {msg}"
                        )
                        continue
                    ...
                    logger.info(
                        f"重试发送任务 {task_id} 从 {source_session} 到 {target_session} 的消息"
                    )
                    ...
                    if not valid_messages:
                        logger.warning(
                            f"任务 {task_id} 会话 {source_session} 的消息缓存为空，无法重试"
                        )
                        continue
                    ...
                    if target_platform == "aiocqhttp":
                        ...
                        logger.warning(
                            f"目前重试功能只支持QQ平台，跳过 {target_session}"
                        )
    ...
    async def _validate_retry_prerequisites(
        ...
        if (
            ...
            logger.warning(
                f"任务 {task_id} 会话 {source_session} 的消息缓存已清空，无法重试转发"
            )
            return False
