            ...
            if latest_tag:
                ...
                try:
                    ...
                    if result_check.stdout.strip():
                        cmd = cmd_check
                        print(
                            f"📋 自动检测：基于标签 {latest_tag} 到 {to_tag} 生成 changelog"
                        )
                    else:
                        # 如果没有新提交，获取最近10个提交作为示例
                        cmd = "git log --oneline -10"
                        print(
                            f"📋 警告：{latest_tag} 到 {to_tag} 之间没有新提交，使用最近10个提交"
                        )
                except subprocess.CalledProcessError:
                    cmd = "git log --oneline -20"
                    print("📋 Git命令执行失败，使用最近20个提交")
            else:
                cmd = "git log --oneline -20"  # 如果没有标签，获取最近20个提交
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Examples:")
        print(
            "  python generate_changelog.py 1.3.0                    # 自动检测最新标签"
        )
        print("  python generate_changelog.py 1.3.0 v1.2.0 HEAD       # 指定版本范围")
        print(
            "  python generate_changelog.py 1.3.0 --all v1.0.0      # 为初始版本生成（从项目开始到指定标签）"
        )
